...
export default {
    ...
    async addItem(req, res) {
        try {
            ...
            const existing = req.session.carrinho.find(p => p.id_produto == id_produto);
            if (existing) {
    ...
    async saveEntrega(req, res) {
        try {
            ...
            return res.json({ success: false, message: "Erro ao salvar tipo de entrega" });
        }
    },
    ...
    async savePagamento(req, res) {
        try {
            ...
            return res.json({ success: false, message: "Erro ao salvar forma de pagamento" });
        }
    }
};