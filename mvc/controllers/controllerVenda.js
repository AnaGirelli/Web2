import { Venda, VendaProduto, Produto, Pessoa, Avaliacao } from '../models/index.js';
...
const getCheckoutData = async (req) => {
    ...
    const custoFrete = tipoEntrega === 'ENTREGA' ? 15.00 : 0.00;
    ...
    for (const item of itens) {
        ...
        if (produto && produto.id_vendedor) {
...
export default {
    ...
    async viewCheckout(req, res) {
        try {
            ...
            return res.status(500).send("Erro interno ao carregar tela de finalização.");
        }
    },
    ...
    async confirmarVenda(req, res) {
        try {
            const { itens, subtotal, custoFrete, total, tipoEntrega } = await getCheckoutData(req);
            ...
            return res.status(500).json({ success: false, message: "Erro interno ao processar a venda." });
        }
    },
    ...
    async getPedidosVendedor(req, res) {
        try {
            ...
            const { Venda, VendaProduto, Produto, Pessoa } = await import('../models/index.js');
