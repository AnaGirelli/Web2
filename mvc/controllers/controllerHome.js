...
export default {
    async viewHome(req, res) {
        try {
            ...
            const categoriasFormatadas = categorias.map(cat => ({ id: cat.id_categoria, name: cat.nome_categoria }));
    ...
    async searchProducts(req, res) {
        try {
            ...
            if (!q || q.trim() === '') {
                ...
                const categorias = await Categoria.findAll({ order: [['id_categoria', 'ASC']] });
                const categoriasFormatadas = categorias.map(cat => ({ id: cat.id_categoria, name: cat.nome_categoria }));
                return res.render('home', { produtos, categorias: categoriasFormatadas, categoriaAtual: null });
            }
